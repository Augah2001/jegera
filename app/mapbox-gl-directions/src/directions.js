...
export default class MapboxDirections {
  ...
  _onSingleClick(e) {
    const fixedDestination = [31.05430554369687,-17.785005427353127]
    ...
    if (!origin.geometry) {
      ...
      if (features.length) {
        // Remove any waypoints... (rest of code unchanged)
      } else {
        this.actions.setOriginFromCoordinates(coords);
        this.actions.setDestinationFromCoordinates(fixedDestination); // Set fixed destination
        this._map.flyTo({ center: coords }); // Fly to clicked location (origin)
      }
    }
  }
