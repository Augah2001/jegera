...
import {suggestAPIclient, retrieveAPIclient } from "../configs/mapBoxclient";
...
const MapSearchComponent = () => {
  ...
  const handleChange = (input: string) => {
    ...
    suggestAPIclient.get(`/suggest?q=${input}`).then((res) => {
  ...
  const handleClickResult = (mapbox_id: string) => {
    ...
    retrieveAPIclient.get(`/retrieve/${mapbox_id}`).then((res) => {
      console.log(res.data);
      setSuggestions([] as Suggestion[]);
      setValue('')
